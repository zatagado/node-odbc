...
class Statement {
   ...
   * @param {object} [options] - Optional settings. Use `{ multipleResultSets: true }` to return an array of
   *   result objects (same shape as a single result) when the batch produces multiple result sets (e.g. multiple
   *   SELECTs). Incompatible with `cursor` / `fetchSize`.
   * @param {function} [callback] - The callback function that returns the result. If omitted, uses a Promise.
   */
